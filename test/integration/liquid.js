...
import { liquidClient, lndDataPath } from '../Nodes';
...
describe('liquidclient', () => {
  beforeAll(async () => {
    await liquidLndClient.connect();
  });

  afterAll(async () => {
    liquidLndClient.disconnect();
  });

  test('should calculate payment fees', async () => {
    const calculatePaymentFee = liquidLndClient['calculatePaymentFee'];

    const bigInvoiceAmount = 8754398;
    let invoice = await liquidLndClient.addInvoice(bigInvoiceAmount);
    ...
    invoice = await liquidLndClient.addInvoice(1);
    expect(calculatePaymentFee(invoice.paymentRequest)).toEqual(LndClient['minPaymentFee']);

    invoice = await liquidLndClient.addInvoice(0);
    expect(calculatePaymentFee(invoice.paymentRequest)).toEqual(LndClient['minPaymentFee']);
  });
